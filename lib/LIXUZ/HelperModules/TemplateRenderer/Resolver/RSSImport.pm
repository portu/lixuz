...
package LIXUZ::HelperModules::TemplateRenderer::Resolver::RSSImport;
use Moose;
...
sub get_list
{
    ...
    else
    {
        ...
        $content = $self->c->model('LIXUZDB::LzRssArticle')->search({ deleted => \'!= 1', status => 'Active' }, { order_by => 'pubdate DESC', rows => $limit });
