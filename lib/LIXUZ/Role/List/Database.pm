...
package LIXUZ::Role::List::Database;
...
sub listGetPaginatedResultObject
{
    ...
    {
        $self->c->stash->{ $self->_loptions->{objectName} } = $object;
    }
...
sub listGetResultObject
{
    ...
    my $obj = $self->_loptions->{object};

    my $object = $self->_loptions->{object}->search($search,$settings);
...
{
    my $self = shift;
    return $self->_loptions->{object}->current_source_alias;
}
