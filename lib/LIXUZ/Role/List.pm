...
sub handleListRequest
{
    ...
    {
        $form = $options->{formbuilder};
    }
    elsif ($self->can('formbuilder'))
    {
    ...
    if ($form && $form->submitted)
    {
        ...
        if ($self->_loptions->{searchColumns})
        {
            if ($self->_loptions->{advancedSearch})
            {
                $self->list_advanced_search({
                        string => $query,
                        columns => $self->_loptions->{searchColumns},
                    }, $self->_loptions->{advancedSearch});
            }
            else
            {
                $self->list_search($query, $self->_loptions->{searchColumns});
            }
        }
    ...
    if ($self->_loptions->{template})
    {
        $self->c->stash->{template} = $self->_loptions->{template};
    }

    if ($self->_loptions->{autoSearch})
    {
        return $self->listGetResultObject({ paginate => $self->_loptions->{paginate} });
    }
}
...
{
    my $self = shift;
    my @allowed = @{$self->_loptions->{orderParams}};
...
sub listFolderFilter
{
    ...
    my $type =  $self->_loptions->{folderType} // 'external';
...
sub _lSetOptions
{
    ...
    $self->_lSearchData($self->_buildSearchData);
    $self->_lResultObject(undef);
    $self->_lDataChanged(0);
    if ($self->can('_indexerResultObject'))
    {
        $self->_indexerResultObject(undef);
    }

    if(!exists $options->{autoSearch})
    {
        $options->{autoSearch} = 1;
    }

    $self->_loptions($options);
}
